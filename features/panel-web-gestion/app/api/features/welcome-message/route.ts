...
import {
  getWelcomeMessageRecord,
  normalizeWelcomeMessageConfig,
  saveWelcomeMessageRecord,
} from "@/lib/store";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [record, meta] = await Promise.all([
      getWelcomeMessageRecord(context.guildId),
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const config = normalizeWelcomeMessageConfig(body?.config);

    const saved = await saveWelcomeMessageRecord({
