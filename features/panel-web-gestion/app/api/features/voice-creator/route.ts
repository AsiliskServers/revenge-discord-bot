...
import {
  getVoiceCreatorRecord,
  normalizeVoiceCreatorConfig,
  saveVoiceCreatorRecord,
} from "@/lib/store";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [record, meta] = await Promise.all([
      getVoiceCreatorRecord(context.guildId),
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const config = normalizeVoiceCreatorConfig(body?.config);

    const saved = await saveVoiceCreatorRecord({
